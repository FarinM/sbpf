...
impl MemoryMapping {
    ...
    pub fn initialize(&mut self) -> Result<(), EbpfError> {
        ...
        static INLINE_OFF: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
        if self.initialized
            && !self.disable_address_translation
            && !INLINE_OFF.get_or_init(|| std::env::var_os("SBPF_INLINE_OFF").is_some())
        {
